...
// Some variables
$js_path                            = 'webplate/js/min/';
$css_path                       	= 'webplate/css/';
$icomoon_path                   	= 'webplate/icomoon/';

// Execute JS
$LAB

    // Load required js
    .script($js_path + 'webplate-jquery-1.9.1.min.js').wait()
    .script($js_path + 'webplate-modernizer-2.6.2.min.js').wait()
        ...
        if(($css_extras) && ($css_extras.length > 0)){
            ...
            $.each($split_css_extras, function($index, $file){
                ...
                if($extension == 'css'){
                    ...
                    $('head').append('<link rel="stylesheet" href="'+ $css_path + $file +'" type="text/less" />');
                }
            });
        }
    })
    ...
    // Load the LESS
    .script($js_path + 'webplate-less-1.3.3.min.js').wait(function(){
