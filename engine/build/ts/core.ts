	...
	const core = {
      fadeOut: (obj) => {
		...
		load: {
            ...
            let iconFont = Rocket.helper.setDefault(cockpit.iconFont, '');
            let load = Rocket.helper.setDefault(cockpit.load, []);
   			let pageMatch = false;
   			let queryString = '';
            ...
            // Fix the paths
            for (let i = 0, len = app.css.length; i < len; i++) {
               if (app.css[i].substring(0, 4) !== 'http') {
                  app.css[i] = path.css + app.css[i];
               }
            }
            for (let i = 0, len = app.js.length; i < len; i++) {
